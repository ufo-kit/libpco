        ...
        void *buffer_out, uint32_t size_out)
{
    ...
    check_error_cl(clSetBaudRate(pco->serial_ref, 9600));
    com_out = 0;
    ...
    printf("Start read\n");
    check_error_cl(clSerialRead(pco->serial_ref, (char *) buffer, &size, pco->timeouts.command*2));
    ...
    printf("Read again\n");
    check_error_cl(clSerialRead(pco->serial_ref, (char *) &buffer[sizeof(uint16_t)*2], &size, pco->timeouts.command*2));
...
int main(int argc, char const* argv[])
{
    ...
    unsigned int buffer_size, version;
    char str[256];
    ...
    for (int i = 0; i < pco.num_ports && i < 4; i++) {
        ...
        check_error_cl(clGetSupportedBaudRates(pco.serial_refs[i], baudrates));
        printf("   Baud rates: ");
        for (int bidx = 0; bidx < 8 && baudrates[bidx] > 0; bidx++)
            printf("%i ", baudrates[bidx]);
        printf("\n");
    }
    ...
    pco_control_command(&pco, &com, sizeof(com), &resp, sizeof(SC2_Camera_Type_Response));
